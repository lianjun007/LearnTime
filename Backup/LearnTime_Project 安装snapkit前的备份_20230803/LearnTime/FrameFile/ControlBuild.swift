...
struct SettingControl {
    ...
    static func build(control: Array<rowMode>? = [.forward], caption: String? = nil, tips: String? = nil, mode: imageMode? = .basic, label: Array<String>? = nil, text: Array<String>? = nil) -> Dictionary<String, UIView> {
        ...
        settingTable.frame.origin = CGPoint(x: 0, y: (caption == nil) ? 0: captionLabel.frame.maxY + 6)
        ...
        for (index, item) in control!.enumerated() {
            ...
            case .forward:
                ...
                let rowLabel = forward(text: text, index)[1]
                row.addSubview(rowLabel)
                returnDictionary["text\(index + 1)"] = rowLabel
            ...
            rowLabel.text = label?[index] ?? "    "
            rowLabel.font = Font.text(.regular)
            rowLabel.textColor = UIColor.black
            rowLabel.sizeToFit()
            rowLabel.frame.size.width = Screen.basicWidth() - 53
            row.addSubview(rowLabel)
        ...
        if tips != nil {
            tipsLabel.frame.origin = CGPoint(x: Spaced.screen(), y: returnDictionary["\(control!.count)"]!.frame.maxY + (captionLabel.frame.maxY != 0 ? captionLabel.frame.maxY + 6: 0) + 6)
        ...
        settingControl.frame.size.height = !(tips == nil) ? tipsLabel.frame.maxY: settingTable.frame.maxY
...
struct ShowcaseControl {
    ...
    static func rowBuild(originX: CGFloat? = Spaced.screenAuto(), // 原点的X值，默认为Spaced.screenAuto()⚠️未启用
             origin: CGPoint,
             image: UIImage,
             title: String,
             text: String,
        ...
        control.setImage(spliceImage(image, direction: direction), for: .normal)
        ...
        largeTitle.textColor = UIColor.black.withAlphaComponent(0.8)
        largeTitle.font = Font.title2()
        largeTitle.sizeToFit()
        largeTitle.frame.size.width = Screen.basicWidth()
        ...
        smallTitle.textColor = UIColor.black.withAlphaComponent(0.8)
        smallTitle.font = Font.text()
        smallTitle.sizeToFit()
        smallTitle.frame.size.width = Screen.basicWidth()
        smallTitle.isUserInteractionEnabled = false
        control.addSubview(smallTitle)
        // ⚠️这两个标题还有一个未解决的隐患：没有考虑标题字数太长的问题
        
        return [control, largeTitle, smallTitle]
    }
    
    static func spliceImage(_ image: UIImage, direction: Bool) -> UIImage {
        // 裁剪和拼接控件的背景图片
        let image = image
...
//    }
    ...
    largeTitle.text = title
    largeTitle.textColor = UIColor.white.withAlphaComponent(0.8)
    largeTitle.font = Font.title2()
    largeTitle.sizeToFit()
    largeTitle.isUserInteractionEnabled = false
    control.addSubview(largeTitle)
    ...
    smallTitle.textColor = UIColor.white.withAlphaComponent(0.8)
    smallTitle.font = Font.text()
    smallTitle.sizeToFit()
    smallTitle.isUserInteractionEnabled = false
    control.addSubview(smallTitle)
